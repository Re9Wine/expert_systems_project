    ...
    <div class="table">
      ...
      <table>
        ...
        <tr>
          ...
          <th scope="col">Прогноз трат на этот месяц</th>
        ...
        <tr v-for="(item, index) in table.category" :key="index">
          ...
          <td data-label="Сумма">{{ table2.forecast[index] }}</td>
  ...
  mounted() {
    fetch('TransactionCategory/MonthlyConsumption', {
      ...
      headers: {
        ...
        fetch('MoneyTransaction/ForecastByCategoryForTheNextMonth', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          },
            ...
            .then(response => {
              this.table2 = {
                category: response.map((responseData) => responseData.category),
                forecast: response.map((responseData) => responseData.forecast),
              }
              // var nameLengths = response.map(function(name) {
              //     return name.category;
              // });
